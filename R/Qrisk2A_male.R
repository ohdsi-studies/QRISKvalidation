...
#' 
#' @export
createQRISK2AMaleOG10 <- function(
...
plpModelQRISK2A_male_OG_10 <- PatientLevelPrediction::createGlmModel(
  ...
  coefficients = data.frame(
    ...
    coefficient = c(
      # townsend
      0.0323801637634487590000000,
      
      # race/ethnicity
      0, 0, 0.3173321430481919100000000, 0.4738590786081115500000000,
	  0.5171314655968145500000000, 0.1370301157366419200000000, 0.1370301157366419200000000,
	  0.1370301157366419200000000, -0.3885522304972663900000000,
	  -0.3812495485312194500000000, -0.3812495485312194500000000,
	  -0.4064461381650994500000000, -0.2285715521377336100000000,
      
      # smoker low to high
      0, 0.2684479158158020200000000, 0.6307674973877591700000000,
	  0.7178078883378695700000000, 0.8704172533465485100000000,
      
      # binary
      1.0363048000259454000000000, 0.2519953134791012600000000, 0.8359352886995286000000000,
	  0.6603459695917862600000000, 1.3309170433446138000000000, 0.9454348892774417900000000,
	  0.5986037897136281500000000,
      
      # measurements
      -18.0437312550377270000000000, 0.0236486454254306940000000, 
	  2.5388084343581578000000000, -9.1034725871528597000000000,
	  0.1684397636136909500000000, 0.0105003089380754820000000,
      
      # interactions - age 1
      0.6186864699379683900000000, 1.5522017055600055000000000,
	  2.4407210657517648000000000, 3.5140494491884624000000000,
	  8.0382925558108482000000000, -1.6389521229064483000000000,
	  8.4621771382346651000000000, 5.4977016563835504000000000,
	  3.3974747488766690000000000, 33.8489881012767600000000000,
	  -140.6707025404897100000000000, 2.0858333154353321000000000,
	  0.0501283668830720540000000, -0.1988268217186850700000000,
        
      # interactions - age 2
      -0.0040893975066796338000000, -0.0056065852346001768000000,
	  -0.0018261006189440492000000, -0.0014997157296173290000000,
	   0.0052471594895864343000000, -0.0179663586193546390000000,
	   0.0092088445323379176000000, 0.0047493510223424558000000,
	  -0.0048113775783491563000000, 0.0627410757513945650000000,
	  -0.2382914909385732100000000, -0.0049971149213281010000000,
	  -0.0000523700987951435090000, -0.0012518116569283104000000
        
                   )  
  ), 
  intercept = 0, 
  mapping = "function(x){ sapply(x, function(x){
  (1 - (0.978794217109680)^exp(x) )
})}", 
  ...
  featureEngineering = list(
    ...
    QRISKvalidation::createCenteringFe(
      centers = data.frame(
        ...
        centerValue = c(
          0.233734160661697, 18.304403305053711,
		  0.146269768476486, 0.140587374567986,
		  4.321151256561279, 130.589752197265620,
		  0.551009356975555
        )
      )),
...
return(plpModelQRISK2A_male_OG_10)
}
